...
class SnapResults(object):
    def __init__(self, view : pya.LayoutView = None, snapPoint : pya.DPoint = None, snappedVertex : pya.DPoint = None, snappedEdge : pya.DEdge = None, snappedShape : pya.DPolygon = None):
        super().__init__()   
        self.view          = view
        self.snapPoint     = snapPoint
        self.snappedVertex = snappedVertex
        self.snappedEdge   = snappedEdge
        self.snappedShape  = snappedShape
        self.snapped       = True if snappedShape else False
        ...
        for hoveredShape in hoverShapes:
            ...
            for i in itemList:
                ...
                if (dist <= minDistance) if isinstance(i, pya.DEdge) else (dist <= detectRange * 2):
                    ...
                    hlShape     = hoveredShape
        self.drawHighlight(snapPoint, detectRange, centerShapes, hlCenter, hlVertex, hlEdge)
        snapResults = SnapResults(self.view, snapPoint, hlVertex, hlEdge, hlShape)
        return snapResults
