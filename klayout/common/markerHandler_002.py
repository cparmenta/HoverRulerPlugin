...
class MarkerTemplate:
    ...
    def transformed(self, trans : pya.DCplxTrans):
        self.data = self.data.transformed(trans)
        return self
        
    def move(self, vector : pya.DVector ):
        self.data = self.data.move(vector)
        return self

def unpackRulerProps(nestedRulerProps):
   
    def flatten(rulerProps):
        for rulerProp in rulerProps:
            if isinstance(rulerProp, list):
                flatten(rulerProp)
            elif isinstance(rulerProp, dict):
                result.append(rulerProp)

    result = []
    flatten([nestedRulerProps])
    print (result)
    return result
        
def unpackMarkProps(nestedMarkProps):
    def flatten(markProps):
        for markProp in markProps:
            if isinstance(markProp, list):
                flatten(markProp)
            elif isinstance(markProp, MarkerTemplate):
                if markProp.data : result.append(markProp)

            elif isinstance(markProp, dict):
                if markProp["data"] : result.append(markProp)

    result = []
    flatten(nestedMarkProps)
    return result
    
def transformMarkProps(markProps : List[Union[MarkerTemplate, List]], trans : pya.DTrans):
    return[markProp.transformed(trans) for markProp in unpackMarkProps(markProps)]

def moveMarkProps( markProps : List[Union[MarkerTemplate, List]], vector : pya.DVector):
    return[markProp.move(vector) for markProp in unpackMarkProps(markProps)]
...
class MarkerHandler:
    ...
    def markPropsAppend(self, markProps):
        for p in unpackMarkProps(markProps):
            self.markPropList.append(p)
        '''
...
class RulerHandler:
    ...
    def rulerPropsAppend(self, rulerProps):
        #for p in unpackRulerProps(rulerProps):
         #   self.rulerPropList.append(p)
